...
for i, rain_image_path in enumerate(rain_image_paths):
    ...
    _, gt_phase, gt_mag_unnorm = generate_fft_YCRCB(groundtruth_filepath)
    _, rain_phase, rain_mag_unnorm = generate_fft_YCRCB(rain_filepath)
...
def construct_stats(maxs, mins, means, stds):
    stats = {}
    stats["mag_Y_max"] = float(maxs[0])
    stats["mag_Cr_max"] = float(maxs[1])
    stats["mag_Cb_max"] = float(maxs[2])
    stats["phase_Y_max"] = float(maxs[3])
    stats["phase_Cr_max"] = float(maxs[4])
    stats["phase_Cb_max"] = float(maxs[5])
    stats["mag_Y_min"] = float(mins[0])
    stats["mag_Cr_min"] = float(mins[1])
    stats["mag_Cb_min"] = float(mins[2])
    stats["phase_Y_min"] = float(mins[3])
    stats["phase_Cr_min"] = float(mins[4])
    stats["phase_Cb_min"] = float(mins[5])
    stats["mag_Y_mean"] = float(means[0])
    stats["mag_Cr_mean"] = float(means[1])
    stats["mag_Cb_mean"] = float(means[2])
    stats["phase_Y_mean"] = float(means[3])
    stats["phase_Cr_mean"] = float(means[4])
    stats["phase_Cb_mean"] = float(means[5])
    stats["mag_Y_std"] = float(stds[0])
    stats["mag_Cr_std"] = float(stds[1])
    stats["mag_Cb_std"] = float(stds[2])
    stats["phase_Y_std"] = float(stds[3])
    stats["phase_Cr_std"] = float(stds[4])
    stats["phase_Cb_std"] = float(stds[5])
    keys = []
    vals = [] 
...
output_csv_rain = "statistics/rain_fft_statistics_log_YCrCb.csv"
output_csv_diff = "statistics/diff_fft_statistics_log_YCrCb.csv"
