...
class Model:
    def __init__(self, x_rain, is_training, batch_size):
        ...
        self.weight = w//4
        self.height = h//4
        self.rain_res = self.generator(x_rain, is_training, tf.compat.v1.AUTO_REUSE)
    ...
    def generator(self, rain, is_training, reuse):
        with tf.compat.v1.variable_scope('generator', reuse=reuse):
            ...
            with tf.compat.v1.variable_scope('LSTM4'):
            ...
            with tf.compat.v1.variable_scope('residual_memory'):
			    ...
			    ############ rain2
			    ############ rain2
                with tf.compat.v1.variable_scope('upRMM4_2'):
			    ...
			    ############ rain
			    ############ rain
                with tf.compat.v1.variable_scope('upRMM4_1'):
                    rain4to2 = deconv_layer(
                        rain4to2, [3, 3, 64, 64], [self.batch_size, self.weight*4, self.height*4, 64], 2)
                    rain4to2 = prelu(rain4to2)
                with tf.compat.v1.variable_scope('upRMM2_1'):
            ...
            for n in range(17):
                ...
                ############ rain4
                ############ rain4
                with tf.compat.v1.variable_scope('URAB4_{}'.format(n)):
                    with tf.compat.v1.variable_scope('down4_{}'.format(n+1)):
                        x_rnn = conv_layer(res4_in, [3, 3, 64, 64], 2) 
                        x_rnn = prelu(x_rnn)
                    res_short = res_input = x_rnn
                    for m in range(1):
                        with tf.compat.v1.variable_scope('group_{}_RCAB{}'.format(n+1,m+1)):
                            res_input = self.RCAB(res_input, 4)
                    with tf.compat.v1.variable_scope('up_{}'.format(n+1)):
                ...
                ############ rain2
                ############ rain2
                with tf.compat.v1.variable_scope('URAB2_{}'.format(n)):
                    with tf.compat.v1.variable_scope('upURAB4_2_{}'.format(n)):
                    ...
                    with tf.compat.v1.variable_scope('down2_{}'.format(n+1)):
                ...
                ############ rain
                ############ rain
                with tf.compat.v1.variable_scope('URAB_{}'.format(n)):
                    with tf.compat.v1.variable_scope('upURAB4to1_{}'.format(n)):
                    ...
                    with tf.compat.v1.variable_scope('upURAB2to1_{}'.format(n)):
                    ...
                    with tf.compat.v1.variable_scope('down_{}'.format(n+1)):
            ...
            with tf.compat.v1.variable_scope('rnn_recon4'):
            ...
            with tf.compat.v1.variable_scope('rnn_recon2'):
            ...
            with tf.compat.v1.variable_scope('rnn_recon'):
                res_in = deconv_layer(
            ...
            with tf.compat.v1.variable_scope('up4_2'):
                res4_2 = deconv_layer(
                    res4_in, [3, 3, 64, 64], [self.batch_size, self.weight*2, self.height*2, 64], 2)#2
                res4_2 = prelu(res4_2)
            with tf.compat.v1.variable_scope('up2_1'):
            ...
            with tf.compat.v1.variable_scope('rnn5'):
            ...
            with tf.compat.v1.variable_scope('rnn7'):
    ...
    def RCAB(self, input, reduction):
        ...
        f = tf.compat.v1.layers.conv2d(input, channel, 3, padding='same', activation=prelu)  # (B, W, H, C)tf.nn.relu
        f = tf.compat.v1.layers.conv2d(f, channel, 3, padding='same')  # (B, W, H, C)
        x = tf.reduce_mean(f, axis=(1, 2), keepdims=True)  # (B, 1, 1, C)
        x = tf.compat.v1.layers.conv2d(x, channel // reduction, 1, activation=prelu)  # (B, 1, 1, C // r)
        x = tf.compat.v1.layers.conv2d(x, channel, 1, activation=tf.nn.sigmoid)  # (B, 1, 1, C)
    ...
    def downscale2(self, x):
        ...
        downscaled = tf.nn.conv2d(
            x, filters=weight, strides=[1, K, K, 1], padding='SAME')
        return downscaled
